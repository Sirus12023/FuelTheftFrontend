...
// Updated Alert interface based on backend changes
interface Alert {
  type: string;
  timestamp: string;
  description: string;
  location: { latitude: number; longitude: number }; // changed from lat/long to latitude/longitude
  bus: {
    id: string;
    registrationNumber: string; // changed from registrationNo to registrationNumber
    driverName: string; // changed from driver to driverName
    routeName: string; // changed from route to routeName
  };
}
...
const BusEvents: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        // Backend endpoint changed to /alerts (no /all)
  ...
  const busSuggestions = useMemo(() => {
    // registrationNumber instead of registrationNo
    const buses = alerts
  ...
  const filteredAlerts = useMemo(() => {
    ...
    return alerts.filter((alert) => {
      // Defensive: check alert.bus and alert.bus.registrationNumber
      const busReg = alert.bus && typeof alert.bus.registrationNumber === "string"
        ? alert.bus.registrationNumber
        : "";
      const selectedBusSafe = typeof selectedBus === "string" ? selectedBus : "";

      // Only match if both are non-empty strings
      const busMatch =
        busReg &&
        selectedBusSafe &&
        busReg.toLowerCase().includes(selectedBusSafe.toLowerCase());
